...
- **Use the project vocabulary.** Consult [`doc/terminology.md`](doc/terminology.md) when a comment or document introduces a recurring state or lifecycle concept. Prefer `persisted` when describing stored data, state the lifecycle boundary separately when needed, and preserve existing public API names.
