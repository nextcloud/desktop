...
class TestSharingDialog : public QObject
{
  ...
  private Q_SLOTS:
    ...
    void opensRecipientPermissionEditor()
    {
        ...
        const auto dialogX = permissionDialog->property("x").toReal();
        ...
        QVERIFY(dialogTitle);
        QCOMPARE(dialogTitle->property("text").toString(), QStringLiteral("Permissions for Alice"));
        const auto closeButton = dialogContent->findChild<QObject *>(QStringLiteral("closeRecipientPermissionDialogButton"));
        QVERIFY(closeButton);
        QVERIFY(closeButton->property("visible").toBool());
