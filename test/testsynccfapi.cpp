...
void setPinState(const QString &path, PinState state, cfapi::SetPinRecurseMode mode)
{
    ...
    const auto handle = cfapi::handleForPath(p, "void setPinState(const QString &path, PinState state, cfapi::SetPinRecurseMode mode)");
    Q_ASSERT(handle);
...
private slots:
    ...
    void testPinStateLocals()
    {
        ...
        QVERIFY(fakeFolder.syncOnce());
        QVERIFY(!vfs->pinState("onlinerenamed2/file1rename", "testPinStateLocals"));
