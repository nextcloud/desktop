...
VOID DokanUseStdErr(BOOL Status) { g_UseStdErr = Status; }

VOID DokanDebugMode(BOOL Status) { g_DebugMode = Status; }
...
NewDokanInstance() {
  ...
  /*
  FILE *fp;
  fp = fopen("C:\\Users\\poncianoj\\aa.txt", "a+");
  fprintf(fp, "%s\n", "n-1");
  fclose(fp);
  */
...
int DokanMain(PDOKAN_OPTIONS DokanOptions,
  ...
  if (DokanOptions->Options & DOKAN_OPTION_NETWORK &&
	  ...
	  /*fp = fopen("C:\\Users\\poncianoj\\aa.txt", "a+");
	  fprintf(fp, "%s\n", "DokanMain ::test DOKAN_OPTION_NETWORK");
	  fclose(fp);
	  */
  ...
  if (DokanOptions->Version < DOKAN_MINIMUM_COMPATIBLE_VERSION) {
	  ...
	  /*fp = fopen("C:\\Users\\poncianoj\\aa.txt", "a+");
	  fprintf(fp, "%s\n", "DokanMain ::test DOKAN_MINIMUM_COMPATIBLE_VERSION");
	  fclose(fp);
	  */
  ...
  /*
  fp = fopen("C:\\Users\\poncianoj\\aa.txt", "a+");
  fprintf(fp, "%s\n", "DokanMain ::test CheckAllocationUnitSectorSize");
  fclose(fp);
  */
  ...
  /*
  fp = fopen("C:\\Users\\poncianoj\\aa.txt", "a+");
  fprintf(fp, "%s\n", "DokanMain ::test before CreateFile");
  fclose(fp);
  */
  ...
  /*fp = fopen("C:\\Users\\poncianoj\\aa.txt", "a+");
  fprintf(fp, "%s\n", "DokanMain ::test after CreateFile");
  fclose(fp);
  */
  ...
  */
  ...
  /*fp = fopen("C:\\Users\\poncianoj\\aa.txt", "a+");
  fprintf(fp, "d3, instance->MountPoint: %ls -> %ls : instance->DeviceName \n", instance->MountPoint, instance->DeviceName);
  fclose(fp);
  */
  ...
  */
  ...
  //char ccc; scanf("%c", &ccc);

  // wait for thread terminations
  WaitForMultipleObjects(threadNum, threadIds, TRUE, INFINITE);

  //WaitForMultipleObjects(threadNum, threadIds, TRUE, 1000);

  //WaitForMultipleObjects(2, changeNotifications, false, INFINITE);
...
UINT __stdcall DokanLoop(PDOKAN_INSTANCE DokanInstance) {
...
BOOL DokanGetMountPointList(PDOKAN_CONTROL list, ULONG length,
  ...
  PDOKAN_CONTROL dokanControl =
      (PDOKAN_CONTROL)malloc(DOKAN_MAX_INSTANCES * sizeof(*dokanControl));
...
void DokanMapKernelToUserCreateFileFlags(
  ...
  if (outFileAttributesAndFlags) {
    ...
    *outFileAttributesAndFlags = FileAttributes;
    ...
    DokanMapKernelBit(*outFileAttributesAndFlags, CreateOptions,
                      (DWORD)8388608, FILE_SESSION_AWARE);
