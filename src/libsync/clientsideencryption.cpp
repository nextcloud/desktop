...
void FolderMetadata::setupExistingMetadata(const QByteArray& metadata)
{
  ...
  */
  ...
  if (_metadataKey.isEmpty()) {
      qCDebug(lcCse()) << "Migrating from v1 to v1.1";
  ...
  for (auto it = files.constBegin(), end = files.constEnd(); it != end; it++) {
      ...
      auto decryptedFile = decryptJsonObject(encryptedFile, _metadataKey);
      ...
      file.fileVersion = decryptedFileObj["version"].toDouble();
...
QByteArray FolderMetadata::encryptedMetadata() {
    ...
    if (!isNewMetadata) {
        // no need to create new metadataKey as we are setting it for the first time
        _metadataKey = EncryptionHelper::generateRandom(16);
    }
    ...
    const auto metadatKeyEncrypted = encryptData(_metadataKey.toBase64());

    QJsonObject metadata = {
      {metadataKeyJsonKey, QJsonValue::fromVariant(metadatKeyEncrypted)},
      {"version", 1.1}
    };
    ...
    for (auto it = _files.constBegin(), end = _files.constEnd(); it != end; it++) {
        ...
        // if we are migrating, version should be 1.1, since we removed some fields from the encrypted file
        encrypted.insert("version", qMax<double>(it->fileVersion, 1.1));
...
void FolderMetadata::addEncryptedFile(const EncryptedFile &f) {
    ...
    auto fileCopy = f;
    fileCopy.fileVersion = 1.1;

    _files.append(fileCopy);
}
...
bool FolderMetadata::moveFromFileDropToFiles()
{
    ...
    for (auto it = _fileDrop.constBegin(); it != _fileDrop.constEnd(); ++it) {
        ...
        file.fileVersion = decryptedFileObject["version"].toDouble();
