...
bool redirectKeepsCredentials(const QUrl &origin, const QUrl &target, const QUrl &accountUrl)
{
    ...
    if (_followRedirects && !redirectUrl.isEmpty()) {
        ...
        if (requestedUrl.scheme() == QLatin1String("https") && redirectUrl.scheme() == QLatin1String("http")) {
            ...
            if (_followRedirects) {
                ...
                if (!redirectKeepsCredentials(requestedUrl, redirectUrl, _account->url())) {
