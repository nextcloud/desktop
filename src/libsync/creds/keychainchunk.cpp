...
namespace KeychainChunk {
    ...
    QKeychain::Error Job::error() const { return _error; }

    QString Job::errorString() const { return _errorString; }

    QByteArray Job::binaryData() const { return _chunkBuffer; }

    QString Job::textData() const { return _chunkBuffer; }

    bool Job::insecureFallback() const { return _insecureFallback; }

#if defined(KEYCHAINCHUNK_ENABLE_INSECURE_FALLBACK)
    void Job::setInsecureFallback(bool insecureFallback) { _insecureFallback = insecureFallback; }
#endif

    bool Job::autoDelete() const { return _autoDelete; }

    void Job::setAutoDelete(bool autoDelete) { _autoDelete = autoDelete; }
    ...
    void WriteJob::slotWriteJobDone(QKeychain::Job *incomingJob)
    {
        ...
        if (writeJob) {
            ...
            if (writeJob->error() != NoError) {
                qCWarning(lcKeychainChunk)
                    << "Error while writing" << writeJob->key() << "chunk" << writeJob->errorString();
                _chunkBuffer.clear();
            }
        }
        ...
        if (!_chunkBuffer.isEmpty()) {
            ...
            if (_chunkCount > KeychainChunk::MaxChunks) {
                qCWarning(lcKeychainChunk) << "Maximum chunk count exceeded while writing" << writeJob->key() << "chunk"
                                           << QString::number(index) << "cutting off after"
                                           << QString::number(KeychainChunk::MaxChunks) << "chunks";
            ...
            const QString kck = _account
                ? AbstractCredentials::keychainKey(_account->url().toString(), keyWithIndex, _account->id())
                : keyWithIndex;
            ...
            // only add the key's (sub)"index" after the first element, to stay compatible with older versions and
            // non-Windows
            job->setKey(kck);
    ...
    void ReadJob::start()
    {
        ...
        const QString kck = _account ? AbstractCredentials::keychainKey(
                                _account->url().toString(), _key, _keychainMigration ? QString() : _account->id())
                                     : _key;
    ...
    void ReadJob::slotReadJobDone(QKeychain::Job *incomingJob)
    {
        ...
        if (readJob->error() == NoError && !readJob->binaryData().isEmpty()) {
            ...
            if (_chunkCount < KeychainChunk::MaxChunks) {
                ...
                const QString kck = _account ? AbstractCredentials::keychainKey(_account->url().toString(),
                                        keyWithIndex, _keychainMigration ? QString() : _account->id())
                                             : keyWithIndex;
                ...
                qCWarning(lcKeychainChunk) << "Maximum chunk count for" << readJob->key() << "reached, ignoring after"
                                           << KeychainChunk::MaxChunks;
            }
#endif
        } else {
            ...
            if (!readJob->insecureFallback()) { // If insecureFallback is set, the next test would be pointless
                if (_retryOnKeyChainError
                    && (readJob->error() == QKeychain::NoBackendAvailable
                        || readJob->error() == QKeychain::OtherError)) {
                    // Could be that the backend was not yet available. Wait some extra seconds.
                    // (Issues #4274 and #6522)
                    // (For kwallet, the error is OtherError instead of NoBackendAvailable, maybe a bug in QtKeychain)
                    qCInfo(lcKeychainChunk)
                        << "Backend unavailable (yet?) Retrying in a few seconds." << readJob->errorString();
            ...
            if (readJob->error() != QKeychain::EntryNotFound
                ...
                qCWarning(lcKeychainChunk) << "Unable to read" << readJob->key() << "chunk"
                                           << QString::number(_chunkCount) << readJob->errorString();
            }
        }
    ...
    void DeleteJob::slotDeleteJobDone(QKeychain::Job *incomingJob)
    {
        ...
        if (deleteJob->error() == NoError) {
            ...
            if (_chunkCount < KeychainChunk::MaxChunks) {
                ...
                qCWarning(lcKeychainChunk) << "Maximum chunk count for" << deleteJob->key() << "reached, ignoring after"
                                           << KeychainChunk::MaxChunks;
            }
#endif
        } else {
            if (deleteJob->error() != QKeychain::EntryNotFound
                ...
                qCWarning(lcKeychainChunk) << "Unable to delete" << deleteJob->key() << "chunk"
                                           << QString::number(_chunkCount) << deleteJob->errorString();
            }
        }
