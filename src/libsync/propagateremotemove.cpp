    ...
    if (_item->isDirectory()) {
        ...
        if (!_nestedItems.isEmpty()) {
            for (const auto &nestedItem : _nestedItems) {
                if (!nestedItem->_encryptedFileName.isEmpty()) {
                    // when renaming non-encrypted folder that contains encrypted folder, nested files of its encrypted
                    // folder are incorrectly displayed in the Settings dialog encrypted name is displayed instead of a
                    // local folder name, unless the sync folder is removed, then added again and re-synced we are
                    // fixing it by modifying the "_encryptedFileName" in such a way so it will have a renamed root path
                    // at the beginning of it as expected corrected "_encryptedFileName" is later used in
                    // propagator()->updateMetadata() call that will update the record in the Sync journal DB

                    const auto path = nestedItem->_file;
                    const auto slashPosition = path.lastIndexOf('/');
                    const auto parentPath = slashPosition >= 0 ? path.left(slashPosition) : QString();
                    ...
                    const auto remoteParentPath =
                        parentRec._e2eMangledName.isEmpty() ? parentPath : parentRec._e2eMangledName;

                    const auto lastSlashPosition = nestedItem->_encryptedFileName.lastIndexOf('/');
                    const auto encryptedName =
                        lastSlashPosition >= 0 ? nestedItem->_encryptedFileName.mid(lastSlashPosition + 1) : QString();

                    if (!encryptedName.isEmpty()) {
                        nestedItem->_encryptedFileName = remoteParentPath + "/" + encryptedName;
                    }
                }

                SyncJournalFileRecord oldRecord;
                if (!propagator()->_journal->getFileRecord(nestedItem->_originalFile, &oldRecord)) {
                    qCWarning(lcPropagateRemoteMove) << "could not get file from local DB" << nestedItem->_originalFile;
                    done(
                        SyncFileItem::NormalError, tr("could not get file %1 from local DB").arg(nestedItem->_originalFile));
                    return;
                }
                auto &vfs = propagator()->syncOptions()._vfs;
                auto pinState = vfs->pinState(nestedItem->_originalFile);

                const auto targetFile = propagator()->fullLocalPath(nestedItem->_renameTarget);

                if (QFileInfo::exists(targetFile)) {
                    // Delete old db data.
                    if (!propagator()->_journal->deleteFileRecord(nestedItem->_originalFile)) {
                        qCWarning(lcPropagateRemoteMove)
                            << "could not delete file from local DB" << nestedItem->_originalFile;
                        done(SyncFileItem::NormalError,
                            tr("Could not delete file record %1 from local DB").arg(nestedItem->_originalFile));
                        return;
                    }
                    if (!vfs->setPinState(nestedItem->_originalFile, PinState::Inherited)) {
                        qCWarning(lcPropagateRemoteMove)
                            << "Could not set pin state of" << nestedItem->_originalFile << "to inherited";
                    }
                }

                SyncFileItem newItem(*nestedItem);
                newItem._type = nestedItem->_type;
                if (oldRecord.isValid()) {
                    newItem._checksumHeader = oldRecord._checksumHeader;
                    if (newItem._size != oldRecord._fileSize) {
                        qCWarning(lcPropagateRemoteMove)
                ...
                const auto result = propagator()->updateMetadata(newItem);
                if (!result) {
                    done(SyncFileItem::FatalError, tr("Error updating metadata for nested item: %1").arg(result.error()));
                    return;
                } else if (*result == Vfs::ConvertToPlaceholderResult::Locked) {
                    done(SyncFileItem::SoftError, tr("The nested file %1 is currently in use").arg(newItem._file));
                    return;
                }
                if (pinState && *pinState != PinState::Inherited
                    && !vfs->setPinState(newItem._renameTarget, *pinState)) {
                    done(SyncFileItem::NormalError, tr("Error setting pin state for nested item"));
                    return;
                }
            }
        }
