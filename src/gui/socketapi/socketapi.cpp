...
QObject *findWidget(const QString &queryString, const QList<QWidget *> &widgets = QApplication::allWidgets())
{
    ...
    if (queryString.contains('>')) {
        ...
        foundWidget = std::find_if(objects.constBegin(), objects.constEnd(),
            [&](QObject *widget) { return widget->objectName() == objectName; });
    } else {
        QList<QObject *> matches;
        std::copy_if(objects.constBegin(), objects.constEnd(), std::back_inserter(matches),
            [&](QObject *widget) { return widget->inherits(queryString.toLatin1()); });
    ...
    : QObject(parent)
{
    ...
    if (Utility::isWindows()) {
        socketPath = QLatin1String(R"(\\.\pipe\)") + QLatin1String(APPLICATION_EXECUTABLE) + QLatin1String("-")
        ...
        auto _system = [](const QString &cmd, const QStringList &args) {
            ...
            if (!process.waitForFinished()) {
                qCWarning(lcSocketApi) << "Failed to load shell extension:" << cmd << args.join(" ")
                                       << process.errorString();
            } else {
                qCInfo(lcSocketApi) << (process.exitCode() != 0 ? "Failed to load" : "Loaded")
                                    << "shell extension:" << cmd << args.join(" ") << process.readAll();
            }
        };
        // Add it again. This was needed for Mojave to trigger a load.
        _system(QStringLiteral("pluginkit"),
            {QStringLiteral("-a"), QStringLiteral("%1Contents/PlugIns/FinderSyncExt.appex/").arg(bundlePath)});
        ...
        _system(QStringLiteral("pluginkit"),
            {QStringLiteral("-e"), QStringLiteral("use"), QStringLiteral("-i"),
                QStringLiteral(APPLICATION_REV_DOMAIN ".FinderSyncExt")});
    ...
    if (!info.dir().exists()) {
        ...
        if (result) {
            QFile::setPermissions(
                socketPath, QFile::Permissions(QFile::ReadOwner + QFile::WriteOwner + QFile::ExeOwner));
        }
    }
...
void SocketApi::slotNewConnection()
{
    ...
    for (Folder *f : FolderMan::instance()->map()) {
        if (f->canSync()) {
            ...
            qCInfo(lcSocketApi) << "Trying to send SocketAPI Register Path Message -->" << message << "to"
                                << listener->socket;
...
void SocketApi::slotReadSocket()
{
    ...
    while (socket->canReadLine()) {
        ...
        const int indexOfMethod = [&] {
            ...
            if (command.startsWith("ASYNC_")) {
                ...
                functionWithArguments +=
                    QByteArrayLiteral("V2_") + command.mid(3) + QByteArrayLiteral("(QSharedPointer<SocketApiJobV2>)");
            } else {
            ...
            if (out == -1) {
                listener->sendError(
                    QStringLiteral("Function %1 not found").arg(QString::fromUtf8(functionWithArguments)));
            }
            ASSERT(out != -1)
            return out;
        }();
        ...
        if (command.startsWith("ASYNC_")) {
            ...
            auto socketApiJob =
                QSharedPointer<SocketApiJob>(new SocketApiJob(jobId.toString(), listener, json), &QObject::deleteLater);
            if (indexOfMethod != -1) {
                staticMetaObject.method(indexOfMethod)
                    .invoke(this, Qt::QueuedConnection, Q_ARG(QSharedPointer<SocketApiJob>, socketApiJob));
            } else {
            ...
            if (indexOfMethod != -1) {
                staticMetaObject.method(indexOfMethod)
                    .invoke(this, Qt::QueuedConnection, Q_ARG(QSharedPointer<SocketApiJobV2>, socketApiJob));
            } else {
                qCWarning(lcSocketApi) << "The command is not supported by this version of the client:" << command
                                       << "with argument:" << argument;
...
void SocketApi::slotUnregisterPath(const QString &alias)
{
    ...
    if (f)
        broadcastMessage(
            buildMessage(QLatin1String("UNREGISTER_PATH"), removeTrailingSlash(f->path()), QString()), true);
...
void SocketApi::slotUpdateFolderView(Folder *f)
{
    ...
    if (f) {
        ...
        if (f->syncResult().status() == SyncResult::SyncPrepare || f->syncResult().status() == SyncResult::Success
            || f->syncResult().status() == SyncResult::Paused || f->syncResult().status() == SyncResult::Problem
            || f->syncResult().status() == SyncResult::Error || f->syncResult().status() == SyncResult::SetupError) {
            ...
            qCDebug(lcSocketApi) << "Not sending UPDATE_VIEW for" << f->alias() << "because status() is"
                                 << f->syncResult().status();
        }
    }
}
...
void SocketApi::command_RETRIEVE_FILE_STATUS(const QString &argument, SocketListener *listener)
{
    ...
    const QString message =
        QLatin1String("STATUS:") % statusString % QLatin1Char(':') % QDir::toNativeSeparators(argument);
    listener->sendMessage(message);
}
...
{
    // listener->sendMessage(QLatin1String("SHARE_MENU_TITLE:") + tr("Share with %1", "parameter is
    // Nextcloud").arg(Theme::instance()->appNameGUI()));
...
void SocketApi::command_EDIT(const QString &localFile, SocketListener *listener)
{
    ...
    auto *job = new JsonApiJob(fileData.folder->accountState()->account(),
        QLatin1String("ocs/v2.php/apps/files/api/v1/directEditing/open"), this);
...
class GetOrCreatePublicLinkShare : public QObject
{
    Q_OBJECT
public:
    GetOrCreatePublicLinkShare(const AccountPtr &account, const QString &localFile, QObject *parent)
        : QObject(parent)
        , _account(account)
        ...
        , _localFile(localFile)
    {
        ...
        connect(&_shareManager, &ShareManager::linkShareRequiresPassword, this,
            &GetOrCreatePublicLinkShare::linkShareRequiresPassword);
...
private slots:
    ...
    {
        bool ok = false;
        QString password = QInputDialog::getText(nullptr, tr("Password for share required"),
            tr("Please enter a password for your link share:"), QLineEdit::Normal, QString(), &ok);
    ...
    void serverError(int code, const QString &message)
    {
        ...
        QMessageBox::warning(nullptr, tr("Sharing error"),
...
class GetOrCreatePublicLinkShare : public QObject
{
    Q_OBJECT
public:
    GetOrCreatePublicLinkShare(const AccountPtr &, const QString &, std::function<void(const QString &link)>, QObject *)
    {
    }
...
void SocketApi::command_COPY_PUBLIC_LINK(const QString &localFile, SocketListener *)
{
    ...
    connect(job, &GetOrCreatePublicLinkShare::done, this, [](const QString &url) { copyUrlToClipboard(url); });
...
void SocketApi::fetchPrivateLinkUrlHelper(
    const QString &localFile, const std::function<void(const QString &url)> &targetFun)
{
    auto fileData = FileData::get(localFile);
    if (!fileData.folder) {
        qCWarning(lcSocketApi) << "Unknown path" << localFile;
    ...
    fetchPrivateLinkUrl(fileData.folder->accountState()->account(), fileData.serverRelativePath, record.numericFileId(),
        this, targetFun);
}
...
void SocketApi::command_RESOLVE_CONFLICT(const QString &localFile, SocketListener *)
{
    ...
    const auto baseRelativePath =
        fileData.folder->journalDb()->conflictFileBaseName(fileData.folderRelativePath.toUtf8());
...
void SocketApi::command_MOVE_ITEM(const QString &localFile, SocketListener *)
{
    ...
    const auto target = QFileDialog::getSaveFileName(nullptr, tr("Select new location …"), defaultDirAndName, QString(),
...
void SocketApi::sendSharingContextMenuOptions(const FileData &fileData, SocketListener *listener, bool enabled)
{
    ...
    if (!capabilities.shareAPI()
        || !(theme->userGroupSharing() || (theme->linkSharing() && capabilities.sharePublicLink())))
        return;

    // If sharing is globally disabled, do not show any sharing entries.
    // If there is no permission to share for this file, add a disabled entry saying so
    if (isOnTheServer && !record._remotePerm.isNull()
        && !record._remotePerm.hasPermission(RemotePermissions::CanReshare)) {
        listener->sendMessage(QLatin1String("MENU_ITEM:DISABLED:d:")
            + (!record.isDirectory() ? tr("Resharing this file is not allowed")
        ...
        bool canCreateDefaultPublicLink = publicLinksEnabled && !capabilities.sharePublicLinkEnforceExpireDate()
            && !capabilities.sharePublicLinkAskOptionalPassword() && !capabilities.sharePublicLinkEnforcePassword();
...
void SocketApi::command_GET_MENU_ITEMS(const QString &argument, OCC::SocketListener *listener)
{
    ...
    if (syncFolder && files.size() == 1 && syncFolder->accountState()->isConnected()) {
        ...
        const auto isE2eEncryptedPath =
            fileData.journalRecord()._isE2eEncrypted || !fileData.journalRecord()._e2eMangledName.isEmpty();
        ...
        if (isConflict || !isOnTheServer) {
            ...
            const bool canAddToDir =
                ...
                || (fileInfo.isDir()
                    && parentRecord._remotePerm.hasPermission(RemotePermissions::CanAddSubDirectories));
    ...
    if (syncFolder && syncFolder->virtualFilesEnabled() && syncFolder->vfs().socketApiPinStateActionsShown()) {
        ...
        auto makePinContextMenu = [&](bool makeAvailableLocally, bool freeSpace) {
            listener->sendMessage(
                QLatin1String("MENU_ITEM:CURRENT_PIN:d:") + Utility::vfsCurrentAvailabilityText(*combined));
            ...
            listener->sendMessage(QLatin1String("MENU_ITEM:MAKE_ONLINE_ONLY:")
                + (freeSpace ? QLatin1String(":") : QLatin1String("d:")) + Utility::vfsFreeSpaceActionText());
        };
...
void SocketApi::command_ASYNC_SET_WIDGET_PROPERTY(const QSharedPointer<SocketApiJob> &job)
{
    ...
    widget->setProperty(arguments["property"].toString().toUtf8().constData(), arguments["value"]);
...
void SocketApi::command_ASYNC_ASSERT_ICON_IS_EQUAL(const QSharedPointer<SocketApiJob> &job)
{
    ...
    if (!widget) {
        QString message =
            QString(QLatin1String("Object not found: 6: %1")).arg(job->arguments()["queryString"].toString());
        job->reject(message);
        return;
    }
...
{
    _socketListener->sendMessage(_command + QStringLiteral("_RESULT:")
        + QJsonDocument({{QStringLiteral("id"), _jobId}, {QStringLiteral("arguments"), obj}})
