...
namespace Ui {
    namespace IconUtils {
        QPixmap pixmapForBackground(const QString &fileName, const QColor &backgroundColor)
        {
            ...
            const QString pixmapColor =
                backgroundColor.isValid() && !Theme::isDarkColor(backgroundColor) ? "black" : "white";
            ...
            if (!QPixmapCache::find(cacheKey, &cachedPixmap)) {
                if (iconBaseColors.contains(pixmapColor)) {
                    cachedPixmap = QPixmap::fromImage(
                        QImage(QString(Theme::themePrefix) + pixmapColor + QLatin1Char('/') + fileName));
