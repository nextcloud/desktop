...
void FileProviderDomainManager::updateFileProviderDomains()
{
    ...
    for (const auto &accountUserIdAtHost : vfsEnabledAccounts) {
        ...
        if (const auto accountState = AccountManager::instance()->accountFromUserId(accountUserIdAtHost)) { // FIXME: WHY IS ACCOUNT STATE NIL?!
    ...
    for (const auto &remainingDomainUserId : configuredDomains) {
        ...
        const auto accountState = AccountManager::instance()->accountFromUserId(accountId); // FIXME: DOES THIS WORK NOW AFTER MAPPING DOMAIN ID TO ID?
