...
ShareDialog::ShareDialog(QPointer<AccountState> accountState, const QString &sharePath, const QString &localPath,
...
ShareLinkWidget *ShareDialog::addLinkShareWidget(const QSharedPointer<LinkShare> &linkShare)
{
    _linkWidgetList.append(
        new ShareLinkWidget(_accountState->account(), _sharePath, _localPath, _maxSharingPermissions, this));
    ...
    connect(
        this, &ShareDialog::toggleShareLinkAnimation, linkShareWidget, &ShareLinkWidget::slotToggleShareLinkAnimation);
    ...
    // connect(_linkWidgetList.at(index), &ShareLinkWidget::resizeRequested, this,
...
void ShareDialog::initLinkShareWidget()
{
    if (_linkWidgetList.size() == 0) {
        ...
        connect(
            _emptyShareLinkWidget, &ShareLinkWidget::resizeRequested, this, &ShareDialog::slotAdjustScrollWidgetSize);
        connect(this, &ShareDialog::toggleShareLinkAnimation, _emptyShareLinkWidget,
            &ShareLinkWidget::slotToggleShareLinkAnimation);
        connect(_emptyShareLinkWidget, &ShareLinkWidget::createLinkShare, this, &ShareDialog::slotCreateLinkShare);

        connect(_emptyShareLinkWidget, &ShareLinkWidget::createPassword, this,
            &ShareDialog::slotCreatePasswordForLinkShare);
...
void ShareDialog::slotPropfindReceived(const QVariantMap &result)
{
    ...
    if (!privateLinkUrl.isEmpty()) {
        ...
        _privateLinkUrl =
            _accountState->account()->deprecatedPrivateLinkUrl(numericFileId).toString(QUrl::FullyEncoded);
    }
...
void ShareDialog::showSharingUi()
{
    ...
    if (userGroupSharing) {
        _userGroupWidget = new ShareUserGroupWidget(
            _accountState->account(), _sharePath, _localPath, _maxSharingPermissions, _privateLinkUrl, this);
...
void ShareDialog::slotCreatePasswordForLinkShare(const QString &password)
{
    ...
    if (shareLinkWidget) {
        connect(_manager, &ShareManager::linkShareRequiresPassword, shareLinkWidget,
            &ShareLinkWidget::slotCreateShareRequiresPassword);
        connect(shareLinkWidget, &ShareLinkWidget::createPasswordProcessed, this,
            &ShareDialog::slotCreatePasswordForLinkShareProcessed);
...
void ShareDialog::slotCreatePasswordForLinkShareProcessed()
{
    ...
    Q_ASSERT(shareLinkWidget);
    if (shareLinkWidget) {
        disconnect(_manager, &ShareManager::linkShareRequiresPassword, shareLinkWidget,
            &ShareLinkWidget::slotCreateShareRequiresPassword);
        disconnect(shareLinkWidget, &ShareLinkWidget::createPasswordProcessed, this,
            &ShareDialog::slotCreatePasswordForLinkShareProcessed);
    } else {
        qCCritical(lcSharing) << "shareLinkWidget is not a sender!";
    }
...
{
    bool ok = false;
    QString password = QInputDialog::getText(this, tr("Password for share required"),
        tr("Please enter a password for your link share:"), QLineEdit::Password, QString(), &ok);
