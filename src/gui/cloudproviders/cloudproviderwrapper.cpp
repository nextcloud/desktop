...
CloudProviderWrapper::CloudProviderWrapper(
    QObject *parent, Folder *folder, int folderId, CloudProvidersProviderExporter *cloudprovider)
    : QObject(parent)
    , _folder(folder)
{
    GMenuModel *model;
    ...
    cloud_providers_account_exporter_set_icon(
        _cloudProviderAccount, g_icon_new_for_string(APPLICATION_ICON_NAME, nullptr));
    ...
    connect(ProgressDispatcher::instance(), SIGNAL(progressInfo(QString, ProgressInfo)), this,
...
{
    return !Progress::isIgnoredKind(item._status) && item._instruction != CSYNC_INSTRUCTION_EVAL
        ...
        if (!_recentlyChanged.isEmpty()) {
            ...
            for (i = _recentlyChanged.begin(); i != _recentlyChanged.end(); i++) {
                ...
                g_menu_item_set_action_and_target_value(
                    item, "cloudprovider.showfile", g_variant_new_string(fullPath.toUtf8().data()));
...
{
    QString status =
        QString("%1 - %2").arg(_folder->accountState()->stateString(_folder->accountState()->state()), statusText);
...
static void activate_action_open(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    Q_UNUSED(parameter);
...
static void activate_action_openrecentfile(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    Q_UNUSED(action);
    Q_UNUSED(parameter);
    auto *self = static_cast<CloudProviderWrapper *>(user_data);
...
static void activate_action_pause(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    Q_UNUSED(parameter);
    auto *self = static_cast<CloudProviderWrapper *>(user_data);
...
static GActionEntry actions[] = {{"openwebsite", activate_action_open, nullptr, nullptr, nullptr, {0, 0, 0}},
