...
int csync_vio_local_stat(const QString &uri, csync_file_stat_t *buf)
{
    ...
    h = CreateFileW(reinterpret_cast<const wchar_t *>(OCC::FileSystem::longWinPath(uri).utf16()), 0,
    ...
    if (!GetFileInformationByHandle(h, &fileInfo)) {
        errno = GetLastError();
        qCCritical(lcCSyncVIOLocal) << "GetFileInformationByHandle failed on" << uri
                                    << OCC::Utility::formatWinError(errno);
        CloseHandle(h);
        return -1;
    }
