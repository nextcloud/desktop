...
struct ScenarioRunner {
    ...
    ///
    /// Populate the database from the server and mark the fixture tree materialised, stamping the
    /// rows well in the past so a later rewrite shows up as a moved `syncTime`.
    ///
    @discardableResult
    ...
    /// the database, so that `rows rewritten` exposes redundant persistence.
    ///
    ...
    /// Marker the fixture rows are stamped with, so any later `syncTime` belongs to the pass
    /// under measurement.
    ...
    ///
    /// Resolve a remote path to the identifier the enumerator needs, enumerating its ancestors
    /// first when the database has not seen it yet.
    ///
