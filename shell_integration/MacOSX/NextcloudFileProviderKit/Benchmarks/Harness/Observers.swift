...
///
/// Stand-in for the framework's change observer that drains `moreComing` batches to completion and
/// records when each arrived, which is what makes time-to-first-change measurable.
///
final class BenchmarkChangeObserver: NSObject, NSFileProviderChangeObserver {
    ...
    /// Mirrors the batch size the framework suggests, `@objc` so the optional protocol
    /// requirement is visible through the protocol existential.
