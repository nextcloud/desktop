...
#   Benchmarks/compare.sh master
#
# The baseline is built from a detached worktree at <base-ref> with this directory copied in, both in
# release, so the comparison is the production code of two revisions measured by one harness.
...
if ! git -C "$REPO_ROOT" rev-parse --verify --quiet "$BASE_REF" >/dev/null; then
    ...
    echo "  git fetch https://github.com/nextcloud/desktop.git master:refs/remotes/upstream/master" >&2
    exit 1
fi
