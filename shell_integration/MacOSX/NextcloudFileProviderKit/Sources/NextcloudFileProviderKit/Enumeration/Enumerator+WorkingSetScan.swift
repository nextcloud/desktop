...
///
/// How many working-set scan reads may be in flight at once, matching the concurrency the framework
/// already drives for content fetches on this domain.
///
...
extension Enumerator {
    ...
    func scanMaterialisedItemsForRemoteChanges() async -> (
        ...
        /// Reads are issued in waves grouped by remote-path depth, so a directory is always read
        /// before the items it covers and items within one wave can never cover one another.
    ...
    /// returning each paired with its result **in the original order** the caller merges them in.
    ///
