...
{
    NSLog(@"FinderSyncXPCManager: Scheduling reconnect in %lu seconds", (unsigned long)_reconnectDelay);

    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, _reconnectDelay * NSEC_PER_SEC), _connectionQueue, ^{
        [self establishConnection];
    });
...
- (void)askForIcon:(NSString *)path isDirectory:(BOOL)isDirectory
{
    ...
    // Check cache first
    NSString *cachedStatus = _statusCache[path];
    if (cachedStatus) {
        dispatch_async(dispatch_get_main_queue(), ^{
