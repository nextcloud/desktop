...
`owncloud-client-master-${COMMIT_HASH}-windows-${COMPILER}.exe` +
in `C:\CraftRoot\tmp`.
...
`-DCMAKE_PREFIX_PATH=/opt/ownCloud/qt-5.10.1/` - version number may vary.
...
+
qtkeychain must be compiled with the same prefix +
e.g., `-DCMAKE_INSTALL_PREFIX=/Users/path/to/client/../install/`
