...
extension Item {
    ...
    static func trash(
        ...
        var postDeleteMetadata = targetItemNKTrash.toItemMetadata(account: account, wasTrashedLocally: true)
    ...
    static func restoreFromTrash(
        ...
        func finaliseRestore(target: NKFile) async -> (Item, Error?) {
            let restoredItemMetadata = target.toItemMetadata(wasTrashedLocally: false)
        ...
        guard enumerateError == .success, !files.isEmpty, let target = files.first else {
            logger.error("Could not scan restored state of file.", [.error: enumerateError, .url: originalLocation])
