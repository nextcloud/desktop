...
| `src/gui/tray/CurrentAccountHeaderButton.qml` | `src/gui/tray/TrayWindowAccountMenu.qml` (+ `src/gui/tray/UserLine.qml`, geteilt mit anderen Aufrufern, selbst aber schon lange vom Upstream-Pendant abgedriftet — kein reines Duplikat) | Online-Status-Indikator auf dem Avatar (farbiger Punkt) deaktiviert — Ticket `SES-50`; dynamische Menübreite (breitester Eintrag) durch feste `Style.sesAccountMenuWidth` ersetzt; `parentBackgroundColor`-Weiterreichung für kontrastabhängige Hintergründe entfällt (Fork nutzt statisches `Style.ses*`-Farbsystem statt QPalette) | `de066e6b9` (2025-03-11, "gui/tray: Hide resume sync for all/pause sync for all menu item if there are no configured sync folders" — Guard am 2026-08-17 in `TrayWindowAccountMenu.qml` nachgezogen) |
